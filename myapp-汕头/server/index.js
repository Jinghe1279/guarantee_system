...
const formatDateToSQL = (value) => {
    if (!value || value === '' || value === null || value === undefined) return null;
    if (typeof value === 'string') {
        // 如果已经是 YYYY-MM-DD 格式，直接返回
        if (/^\d{4}-\d{2}-\d{2}$/.test(value)) return value;
        // 如果是 ISO 8601 格式 (2026-01-20T16:00:00.000Z)，转换为 YYYY-MM-DD
...
// 规范化 guarantees 数据中的日期
const normalizeGuarantees = (guarantees) => {
    if (!Array.isArray(guarantees)) return [];
    return guarantees.map(g => ({
        ...g,
        start_date: formatDateToSQL(g.start_date),
        end_date: formatDateToSQL(g.end_date)
    }));
};

// 规范化 existing_loans 数据中的日期
const normalizeExistingLoans = (loans) => {
...
const mapMainValues = (payload) => {
    ...
    return [
        ...
        formatDateToSQL(company.established_date) || null,
        ...
        formatDateToSQL(controller.birth_date) || null,
...
const insertChildRows = async (applicationId, payload) => {
    ...
    insertSummary.guarantees = (await insertBatch(
        ...
        (row) => [
            ...
            formatDateToSQL(row.start_date) || null,
            formatDateToSQL(row.end_date) || null,
            row.bank_rate || null,
            row.purpose || null
        ]
    )).inserted;
    ...
    insertSummary.asset_stats = (await insertBatch(
        ...
        (row) => [
            applicationId,
            row.name || null,
            formatDateToSQL(row.buy_time) || null,
            row.buy_price || null,
